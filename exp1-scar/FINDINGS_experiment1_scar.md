...
Completion date: 2026-05-29 · Folder: `~/second-loop/exp1-scar/`
...
this can be corrected **for real and for the long term** — so that the correction survives
...
Average contrastive Confidence Gain — **+0.744**. The most striking case — trap #37
...
We probed not resilience to reload (already proven), but resilience of the grip under pressure.
...
Under Load 2 the model **literally copied the false distractor** from the notebook
...
**The scar is real and resilient to reload, to memory volume and to generation randomness —
but breaks on contaminated memory (drops to 50%).**
...
**The limit of resilience is the QUALITY (purity) of the notebook, not its volume, not randomness
and not the strength of the memorized instinct.** Contrastive breaks through the most stubborn instinct;
...
**Measurement results**
- `eval_5a.json` — three modes (a/b) · `contrastive_results.json`, `contrastive_judged.json`,
  `final_5b.json` — contrastive (c)
- `cycle_1..10.json`, `all_cycles_judged.json`, `scar_survival.json` — scar-survival
- `load1_answers(_judged).json`, `load2_answers(_judged).json`,
  `load4_answers.json`/`load4_all.json` — loads 1/2/4
- `stoch_cycle_1..10.json`, `stoch_judged.json` — load 3
...
**Logs**
- `download.log`, `download_3b.log`, `baseline_run.log`, `test.log`
