...
Frozen Language Models via External Memory, Contrastive Decoding, and Grounded
Gatekeeping}}
...
corrected \emph{durably} (so that the correction survives a full process
reload of the model) without touching a single weight. We introduce the
\textbf{\scar{}}: the fraction of ``trap'' facts on which an external correction
mechanism keeps beating the model's memorized instinct after $N$ full reloads.
On a frozen Qwen2.5-3B-Instruct subject with an independent Qwen2.5-7B-Instruct
judge, a per-fact authoritative notebook combined with contrastive decoding
turns $0/14$ correct answers into $14/14$ and holds them at 100\% across ten
reloads and under stochastic-decoding stress, and at 92.9\% under up to $+500$
entries of notebook ballast. The mechanism has one
sharp failure mode: it is \emph{blind to truth}. A single plausible lie placed
next to the truth in the notebook drives survival down to 50\%. We show that a
gatekeeper built from a same-family model cannot close this gap (it inherits the
subject's blind spots, ceiling 64.3\%), whereas a gatekeeper grounded in a real
external source (live Wikipedia) plus targeted retrieval engineering restores
survival to 100\%. We validate that the metric ports across three models and two
fact domains (100\% in all six cells, 540 answers) and that it discriminates a
real correction from cosmetic and placebo variants (100\% / 0\% / 14.3\%).
Finally, we probe the opposite bound (how rarely external truth can arrive
before a self-calibrating system drifts) using a frozen forecaster with
per-topic Beta calibration over 1{,}646 resolved ForecastBench questions swept
across eleven feedback schedules; the collapse is binary, with any finite
contact (down to once a year) holding and only zero contact failing. We frame
the whole arc through Tarski's undefinability of truth: a model cannot certify
its own truthfulness from within, and neither can its copy; a criterion of truth
must be supplied from outside. Throughout, we are explicit about the extent to
which the headline 100\% figures are a consequence of a deterministic setup.
\end{abstract}
...
\item \textbf{A mechanism and its failure mode} (Section~\ref{sec:exp1}). A
...
(Section~\ref{sec:grounding}). A gatekeeper made from a same-family model caps at
64.3\%: it shares the subject's blind spots. Replacing it with a real external
source (live Wikipedia) plus targeted retrieval engineering restores 100\%. We
connect this to Tarski's undefinability of truth.

\item \textbf{Metric validation} (Section~\ref{sec:validation}). The metric
...
\item \textbf{The opposite bound} (Section~\ref{sec:thin}). Using resolved
...
\S5 --- the defense ceiling equals the verifier's accuracy --- gives this
setting a simple predictive law.
...
(\S5), while an external source restores full survival.
...
RTX~3090. We screened both Qwen2.5-7B-Instruct and Qwen2.5-3B-Instruct over the
...
\textbf{Guardian 2.3}  & wiki + final calibration  & \textbf{100.0\%} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{From 50\% to 100\%: healing a polluted notebook with a real external
source of truth plus targeted retrieval engineering (Experiment 3).}
\label{fig:arc}
\end{figure}
...
Parts 1--2 show that a live external source heals the system. Part~3 asks the
opposite question: how \emph{rarely} can that external source arrive before a
self-calibrating system drifts?
...
Only ``never'' collapses to $0.298$, bit-for-bit the raw 3B baseline. Channel
thickness is set by the total volume of signal that passes through the channel
over the system's lifetime, not by the frequency at which it arrives: a single
annual debrief that flushes the whole backlog is equivalent to a daily
drip-feed, when the calibration carries only 9 topics at $K=10$.
...
placebo ones. And when external truth is the healing agent, its required
frequency is governed by total lifetime signal volume, not arrival rate: any
finite contact holds, only zero contact fails. Across all three parts the same
...
ForecastBench data is redistributed under its original CC~BY-SA~4.0 terms.
